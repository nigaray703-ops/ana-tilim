...
test("builds the immutable source-bound 554-target published catalog with approved audio reuse", () => {
  const catalog = buildRecordingCatalog({ projectRoot });

  assert.equal(catalog.schemaVersion, 1);
  assert.match(catalog.generatedAt, /^\d{4}-\d{2}-\d{2}T/);
  assert.equal(catalog.targets.length, 554);
  assert.deepEqual(
  ...
  assert.equal(catalog.targets.filter((item) => item.initialStatus === "pending-review").length, 552);
  assert.equal(catalog.targets.filter((item) => item.initialStatus === "pending").length, 0);
  ...
  assert.deepEqual(firstTimeTargets, []);
  assert.equal(catalog.targets.find((item) => item.stableId === "vocab:erzimaydu").value, "ئەرزىمەيدۇ");
  assert.equal(catalog.targets.find((item) => item.stableId === "vocab:erzimaydu").playable, true);
  assert.equal(catalog.targets.find((item) => item.stableId === "reading:grammar-person-verbs-1").absoluteOutputPath, catalog.targets.find((item) => item.stableId === "reading:grammar-word-order-1").absoluteOutputPath);
  assert.equal(catalog.targets.find((item) => item.stableId === "reading:sentence-self-introduction-4").absoluteOutputPath, catalog.targets.find((item) => item.stableId === "reading:grammar-copula-2").absoluteOutputPath);
  assert.equal(catalog.targets.find((item) => item.stableId === "reading:grammar-word-order-1").value, "مەن كىتاب ئوقۇيمەن.");
  assert.equal(catalog.targets.find((item) => item.stableId === "reading:grammar-copula-2").value, "مەن ئوقۇغۇچى.");
