(() => {
  ...
  const model = { catalog: [], workspace: null, selectedStableId: null, query: "", category: "all", status: "all", activeRecorder: null, recordingTargetId: null, previewPlan: null, pendingUpload: null, imported: new Map(), playedProduction: new Set(), busy: false };
  ...
  function selectTarget(stableId) { if (model.activeRecorder || model.recordingTargetId || model.busy) { setStatus("正在处理当前录音，请完成后再切换目标。", true); return; } model.selectedStableId = stableId; render(); document.getElementById?.(`target-${encodeURIComponent(stableId)}`)?.focus(); }

  function audioForCurrent(target) { const audio = document.createElement("audio"); audio.controls = true; audio.src = `/api/audio/current/${encodeURIComponent(target.stableId)}`; audio.setAttribute("aria-label", `播放当前音频：${target.value}（${target.stableId}）`); audio.addEventListener("playing", () => { if (model.imported.has(target.stableId)) { model.playedProduction.add(target.stableId); renderDetail(); } }); audio.addEventListener("error", () => setStatus("当前课程音频无法播放，请检查该目标后重试。", true)); return audio; }
  ...
  function renderDetail() {
    ...
    fragment.append(sectionTitle("当前课程音频")); const current = document.createElement("div"); current.className = "audio-card"; current.append(audioForCurrent(target)); const auditActions = document.createElement("div"); auditActions.className = "action-row"; auditActions.append(button("当前音频正确", () => auditCurrent(target.stableId)), button("需要重录", () => setAuditStatus(target.stableId, "needs-rerecord"), "danger")); current.append(auditActions); if (model.imported.get(target.stableId)?.hasBackup && model.playedProduction.has(target.stableId)) current.append(button("确认新版并删除这一条旧版备份", () => finalizeOne(target.stableId), "danger")); fragment.append(current);
  ...
  async function performTargetMutation(stableId, action, success) { if (model.busy) return; model.busy = true; render(); try { await action(); await refresh({ focus: true }); setStatus(success); } catch (error) { setStatus(error.message || "操作失败，请重试。", true); } finally { model.busy = false; render(); } }
  ...
  async function startRecording() {
    if (model.activeRecorder || model.recordingTargetId || model.busy) return;
    const target = currentTarget(); if (!target) return;
    model.recordingTargetId = target.stableId; render(); setStatus("正在请求麦克风权限…");
    const mimeType = supportedMimeType();
    if (!mimeType || !navigator.mediaDevices?.getUserMedia || !globalThis.MediaRecorder) { model.recordingTargetId = null; render(); setStatus("当前浏览器不支持 WebM 录音，请使用 Chrome 后重试。", true); return; }
    let stream;
    try {
      ...
      recorder.addEventListener("error", () => { terminal = true; releaseTracks(stream); model.activeRecorder = null; model.recordingTargetId = null; render(); setStatus("录音出现错误，请重新录制。", true); });
    ...
    } catch { releaseTracks(stream); model.activeRecorder = null; model.recordingTargetId = null; render(); setStatus("无法使用麦克风：请允许 Chrome 使用麦克风后重试。", true); }
  }
  function stopRecording() { if (model.activeRecorder?.state === "recording") { setStatus("正在完成录音…"); model.activeRecorder.stop(); } }
  async function finishRecording({ chunks, mimeType, stream, stableId }) { releaseTracks(stream); model.activeRecorder = null; model.recordingTargetId = null; const blob = new Blob(chunks, { type: mimeType }); if (!blob.size) { render(); setStatus("没有收到录音内容，请重新录制。", true); return; } model.pendingUpload = { stableId, blob }; render(); await uploadPending(); }
  async function uploadPending() { const pending = model.pendingUpload; if (!pending || model.busy) return; model.busy = true; render(); try { await request(`/api/takes/${encodeURIComponent(pending.stableId)}`, { method: "POST", headers: { "Content-Type": "audio/webm" }, body: pending.blob }); URL.revokeObjectURL?.(pending.previewUrl); model.pendingUpload = null; await refresh({ focus: true }); setStatus("录音已保存为新 take，仍需逐条批准。" ); } catch (error) { setStatus(`${error.message || "上传失败。"} 本地试听保留，可重试上传。`, true); } finally { model.busy = false; render(); } }
  ...
  function planRow(operation) { const item = document.createElement("div"); const target = model.catalog.find((candidate) => candidate.stableId === operation.stableId); item.className = "plan-operation"; item.textContent = `${operation.stableId} · ${target?.value || "未知目标"} · ${target?.currentFile || "未提供文件名"} · 旧 SHA ${shortHash(operation.currentSha256)} → 新 SHA ${shortHash(operation.replacementSha256)} · ${operation.targetExisted ? "旧版将备份到本次本机导入批次" : "新目标不含旧版备份"} · 文本以预览时的录音文本校验为准`; return item; }
  function shortHash(value) { return value ? String(value).slice(0, 12) : "无当前文件"; }
  ...
  async function applyImport() { const plan = model.previewPlan; if (!plan || model.busy) return; model.busy = true; elements["apply-import"].disabled = true; try { const result = await jsonRequest("/api/import/apply", { planId: plan.planId }); (result.operations || []).forEach((operation) => model.imported.set(operation.stableId, { id: result.importId, hasBackup: operation.targetExisted === true })); model.previewPlan = null; await refresh({ focus: true }); setStatus("导入完成。请播放当前课程音频后，再逐项确认删除旧版备份。" ); } catch (error) { setStatus(error.message || "导入失败，未清除预览。", true); } finally { model.busy = false; render(); } }
  async function finalizeOne(stableId) { const imported = model.imported.get(stableId); if (!imported?.hasBackup || !model.playedProduction.has(stableId) || model.busy) return; model.busy = true; render(); try { await jsonRequest("/api/import/finalize", { importId: imported.id, stableId }); model.imported.delete(stableId); model.playedProduction.delete(stableId); renderDetail(); setStatus("已确认新版并仅删除这一条已验证的旧版备份。" ); } catch (error) { setStatus(error.message || "无法确认这条旧版备份。", true); } finally { model.busy = false; render(); } }
